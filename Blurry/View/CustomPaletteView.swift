...
class CustomPaletteView : UIView {
    ...
    override init(frame: CGRect) {
        ...
        let slider = UISlider(frame: .zero, primaryAction: UIAction { [weak self] action in
            guard let value = (action.sender as? UISlider)?.value,
                  let existing = self?.currentSaturationValue,
                  abs(CGFloat(value) - existing) >= 0.20
            else { return }
            self?.currentSaturationValue = CGFloat(value)
            sliderLabel.text = "Saturation (\(NumberFormatter.localizedString(from: value as NSNumber, number: .decimal)))"
        })
